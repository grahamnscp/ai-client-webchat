...
// msgBroker maintains the set of active clients and broadcasts messages to the clients.
...
*/
func (h *MsgBroker) Run() {

	// Initialise an AI Chat span
	aiclient.InitAIChat()
	...
	for {
		...
		case client := <-h.register:
			...
			log.Printf("chatapp.Run: client registered %s", client.id)
		...
		case client := <-h.unregister:
			...
			if _, ok := h.clients[client]; ok {
				close(client.send)
				log.Printf("chatapp.Run: client unregistered %s", client.id)
...
func getMessageTemplate(msg *Message) []byte {
	...
	if err != nil {
		log.Fatalf("chatapp.getMessageTemplate: template parsing: %s", err)
	}
	...
	if err != nil {
		log.Fatalf("chatapp.getMessageTemplate: template execution: %s", err)
	}
